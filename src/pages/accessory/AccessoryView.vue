<template>
  ...
  <div class="p-grid">
    <div class="p-col-12">
      <Panel header="Gerenciamento de Acessórios">
        ...
        <DataTable
          ...
          currentPageReportTemplate="Exibindo de {first} a {last} no total de {totalRecords} acessórios."
          :loading="loading"
        >
          <template #empty>
          ...
          <Column header="Condição" field="condition" :sortable="true">
            <template #body="slotProps">
              <span :class="'badge status-' + slotProps.data.condition">{{
                $Util.formatCondition(slotProps.data.condition)
              }}</span>
            </template>
          </Column>
          <Column header="Ações">
...
//models
import Accessory from "../../models/internal/accessory";
//Services
...
export default {
  ...
  methods: {
    showCreate() {
      this.obj = new Accessory();

      this.$store.state.views.accessory.dialogForm = true;
    },
    showUpdate(obj) {
      this.obj = obj;
      this.$store.state.views.accessory.dialogForm = true;
